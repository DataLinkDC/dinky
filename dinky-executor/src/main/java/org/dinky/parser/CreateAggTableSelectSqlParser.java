...
public class CreateAggTableSelectSqlParser extends BaseSingleSqlParser {
    ...
    protected void initializeSegments() {
        ...
        segments.add(
                new SqlSegment("(from)(.+?)( where | on | having | group\\s+by | order\\s+by | agg\\s+by | ENDOFSQL)",
                        "(,|\\s+left\\s+join\\s+|\\s+right\\s+join\\s+|\\s+inner\\s+join\\s+)"));
        segments.add(new SqlSegment("(where|on|having)(.+?)( group\\s+by | order\\s+by | agg\\s+by | ENDOFSQL)",
                "(and|or)"));
