...
import org.zdpx.coder.graph.DataType;
import org.zdpx.coder.graph.InputPortObject;
...
/**
 * 广播算子, 输入有两个数据流, 一般为一个为状态稳定数据流, 一个为数据变化流, 将稳定的数据嫁接到后者的每一帧数据上, 由于目前sql方式需要有关联主键
 * 才可以联表, 所以该算子是为源码方式实现的, 之后会有一个sql方式实现的宏算子.
 *
 * @author Licho Sun
 */
@Slf4j
public class BroadcastOperator extends Operator {
    ...
    private static final TypeName MAP_STATE_DESCRIPTOR_VOID_ROW =
        ParameterizedTypeName.get(Specifications.MAP_STATE_DESCRIPTOR, Specifications.VOID, Specifications.ROW);

    private static final TypeName BROADCAST_CONNECTED_STREAM_ROW_ROW =
        ParameterizedTypeName.get(Specifications.BROADCAST_CONNECTED_STREAM, Specifications.ROW, Specifications.ROW);
    ...
    private static final TypeName READ_ONLY_BROADCAST_STATE_VOID_ROW =
        ParameterizedTypeName.get(READ_ONLY_BROADCAST_STATE, Specifications.VOID, Specifications.ROW);
    ...
    protected void execute() {
        ...
        if (outputPorts.isEmpty() || this.operatorWrapper == null || !(this.getSchemaUtil().getGenerateResult() instanceof CodeJavaBuilder)) {
            log.error("BroadcastOperator information err.");
            return;
        }

        Map<String, Object> parameters = getFirstParameterMap();
        Map<String, String> primaryParams = OperatorParameterUtils.getColumns("primary", parameters);
        Map<String, String> broadcastParams = OperatorParameterUtils.getColumns("broadcast", parameters);
        ...
        postOutput(outputPort, broadcastResultTableName, cls);
    }

    private CodeBlock getCodeBlock(Map<String, String> primaryParams,
        ...
        ParameterizedTypeName singleOutputStreamOperatorTupleN =
            ParameterizedTypeName.get(Specifications.SINGLE_OUTPUT_STREAM_OPERATOR, outputTupleNType);

        ParameterizedTypeName broadcastProcessFunctionRow2TupleN = ParameterizedTypeName
            .get(Specifications.BROADCAST_PROCESS_FUNCTION, Specifications.ROW, Specifications.ROW, outputTupleNType);

        TypeName readOnlyContext = broadcastProcessFunctionRow2TupleN.nestedClass("ReadOnlyContext");
        ...
        final TypeName collectorTupleN = ParameterizedTypeName.get(Specifications.COLLECTOR, outputTupleNType);
        TypeSpec broadcastProcessFunction = getBroadcastProcessFunction(
        ...
        return CodeBlock.builder()
            .addStatement("$1T $4N = $3N.toDataStream($3N.sqlQuery(\"select * from $2N\"))",
                Specifications.DATA_STREAM_ROW, primaryTableName, Specifications.TABLE_ENV, primaryTableDs)
            .addStatement("$1T $4N = $3N.toDataStream($3N.sqlQuery(\"select * from $2N\"))",
                Specifications.DATA_STREAM_ROW, broadTableName, Specifications.TABLE_ENV, broadTableNameDs)
            .addStatement("$2T $1L = new $2T($1S, $3T.VOID, $3T.ROW())"
                , descriptorNameVariable, MAP_STATE_DESCRIPTOR_VOID_ROW, Specifications.TYPES)
            .addStatement(
            ...
            .addStatement("$T $N = $N.process($L)",
                singleOutputStreamOperatorTupleN,
                resultDS,
                connectDS,
                broadcastProcessFunction)
            .addStatement("$1T $2N = $3N.fromDataStream($4N).as($5L)",
                Specifications.TABLE,
                broadcastResultTableName,
                Specifications.TABLE_ENV,
                resultDS,
                columnsNames)
            .addStatement("$N.createTemporaryView($S, $N)",
                Specifications.TABLE_ENV,
                broadcastResultTableName,
                broadcastResultTableName)
            .build();
    ...
    private TypeSpec getBroadcastProcessFunction(String descriptorNameVariable,
        ...
        return TypeSpec.anonymousClassBuilder("")
            .addSuperinterface(ParameterizedTypeName.get(Specifications.BROADCAST_PROCESS_FUNCTION, Specifications.ROW, Specifications.ROW,
                outputTupleNType))
            .addMethod(MethodSpec.methodBuilder("processElement")
                ...
                .addStatement("$T $N = $N.getBroadcastState($N)",
                    READ_ONLY_BROADCAST_STATE_VOID_ROW,
                    broadcastState,
                    CTX,
                    descriptorNameVariable)
                .addStatement("$T $N = $L.get(null)", Specifications.ROW, STATE, broadcastState)
                .beginControlFlow("if ($N != null)", STATE)
                .addCode(codeBlock)
                .endControlFlow()
                .build())
            .addMethod(MethodSpec.methodBuilder("processBroadcastElement")
                .addAnnotation(Override.class)
                .addModifiers(Modifier.PUBLIC)
                .addParameter(Specifications.ROW, VALUE)
                ...
                .addParameter(collectorTupleN, OUT)
                .addException(Exception.class)
                .addStatement("$T $L = $N.getBroadcastState($N)",
    ...
    private final TypeName getStreamOutputTypeName(Map<String, String>... columns) {
        ...
        Arrays.stream(columns).flatMap(t -> t.values().stream()).forEach(t -> {
            ClassName type = OperatorParameterUtils.getClassNameByName(t);
            typeList.add(type);
        });

        outputTupleNType = ParameterizedTypeName.get(typeNClass, typeList.toArray(new ClassName[0]));
    ...
    private CodeBlock createOutput(Map<String, String> primaryColumns,
                                   Map<String, String> broadcastColumns,
                                   TypeName tn) {
    ...
    private String getTableNameByInputPort(InputPortObject<TableInfo> inputPortObject) {
        ...
        if (primaryPseudoData == null) {
            throw new NullPointerException(String.format("can not get Output PseudoData from %s",
                inputPortObject.getConnection().getFromPort().getParent().getOperatorWrapper().getName()));
        }
