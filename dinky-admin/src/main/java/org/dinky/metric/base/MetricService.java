...
public abstract class MetricService<T> {
    ...
    private List<T> metricCaches;

    private int currentPos = -1;

    private boolean isScheduleStart = false;

    private final int JOB_GROUP_COUNT = 4;

    protected abstract T formatFlinkJobMetrics(List<HashMap<String, Object>> metrics);

    public T retrieveAllFlinkJobMetrics() {
        if (this.metricCaches == null) {
            synchronized (this) {
                if (!this.isScheduleStart) {
                    this.isScheduleStart = true;
                    schedule.addSchedule(
                            "retrieve.all.flink.job.metrics",
                            () -> this.runRetrieveAllFlinkJobMetrics(false),
                            new PeriodicTrigger(1, TimeUnit.MINUTES));
                }
            }
            this.runRetrieveAllFlinkJobMetrics(true);
        }
        currentPos = (currentPos + 1) % metricCaches.size();
        return this.metricCaches.get(currentPos);
    }

    private void runRetrieveAllFlinkJobMetrics(boolean isOnlyTaskStatus) {
        List<T> currentMetrics = new ArrayList<>();
        List<JobInstance> jobInstances = jobInstanceService.listAllJobInstances();
        if (isOnlyTaskStatus) {
            T allMetrics = this.runRetrieveAllFlinkJobMetricsWithJobList(jobInstances, true);
            for (int i = 0; i < JOB_GROUP_COUNT; i++) {
                currentMetrics.add(allMetrics);
            }
        } else {
            jobInstances.sort(Comparator.comparing(JobInstance::getId));
            List<List<JobInstance>> groups = new ArrayList<>();
            for (int i = 0; i < JOB_GROUP_COUNT; i++) {
                groups.add(new ArrayList<>());
            }
            for (int i = 0; i < jobInstances.size(); i++) {
                groups.get(i % JOB_GROUP_COUNT).add(jobInstances.get(i));
            }
            for (List<JobInstance> group : groups) {
                currentMetrics.add(this.runRetrieveAllFlinkJobMetricsWithJobList(group, false));
            }
        }
        this.metricCaches = currentMetrics;
    }

    private T runRetrieveAllFlinkJobMetricsWithJobList(List<JobInstance> jobInstances, boolean isOnlyTaskStatus) {
        ...
        for (JobInstance jobInstance : jobInstances) {
            try {
                ...
                if ((!isOnlyTaskStatus) && jobInstance.getStatus().equals(JobStatus.RUNNING.toString())) {
    ...
    private List<HashMap<String, Object>> retrieveTaskManagerMetricsByJobInfoDetail(JobInfoDetail jobInfoDetail) {
        ...
        for (JsonNode taskManager : taskManagers) {
    ...
    private List<HashMap<String, Object>> retrieveJobVerticesMetricsByJobInfoDetail(JobInfoDetail jobInfoDetail) {
        ...
        for (String vertice : flinkAPI.getVertices(jid)) {
            HashMap<String, List<String>> whitelist = this.getWhitelistMetricsFromJobVertices(jobInfoDetail, vertice);
            if (!whitelist.isEmpty()) {
                HashMap<String, String> other = new HashMap<>(Map.of(MetricKeys.DINKY_FLINK_TASK_VERTICE_ID, vertice));
                for (String name : whitelist.keySet()) {
                    List<JsonNode> verticeMetrics = new ArrayList<>();
                    for (List<String> metricGroup : Lists.partition(whitelist.get(name), 5)) {
                        JsonNode groupMetrics = flinkAPI.getJobMetricsData(jid, vertice, String.join(",", metricGroup));
            ...
            metricList.addAll(this.retrieveJobVerticeBackPressureMetricsByJobInfoDetail(jobInfoDetail, vertice));
        }
    ...
    public HashMap<String, Object> retrieveJobInfoMetrics(JobInfoDetail jobInfoDetail) {
        ...
        metrics.put(
                MetricKeys.DINKY_FLINK_TASK_NAME, jobInfoDetail.getInstance().getName());
        metrics.put(
                MetricKeys.DINKY_FLINK_TASK_STATUS, jobInfoDetail.getInstance().getStatus());
        metrics.put(
