...
public class ConfigFile extends Configuration {
    ...
    private void parseLoginEntry(Map<String, List<AppConfigurationEntry>> newConfig)
        ...
        if (newConfig.containsKey(appName)) {
            throw ioException(
                    "Configuration.Error.Can.not.specify.multiple.entries.for.appName", appName);
        }
    ...
    private String match(String expect) throws IOException {
        ...
        switch (lookahead) {
            case StreamTokenizer.TT_EOF:
                throw ioException("Configuration.Error.expected.expect.read.end.of.file.", expect);
    ...
    private IOException ioException(String resourceKey, Object... args) {
        MessageFormat form =
                new MessageFormat(
                        ResourcesMgr.getString(resourceKey, "sun.security.util.AuthResources"));
