...
const DocumentForm: React.FC<DocumentFormProps> = (props) => {
    ...
    const [categoryList, setCategoryList] = useState<DefaultOptionType []>([]);
    const [typeList, setTypeList] = useState<DefaultOptionType []>([]);
    const [subTypeList, setSubTypeList] = useState<DefaultOptionType []>([]);
    const [versionOptions, setVersionOptions] = useState<DefaultOptionType []>([]);


    useEffect(() => {
        let versionOptions: DefaultOptionType [] = []
        let categoryOptions: DefaultOptionType [] = []
        let typeOptions: DefaultOptionType [] = []
        let subTypeOptions: DefaultOptionType [] = []
        /**
         * generate version options
         */
        VERSIONS.map((item) => {
            return versionOptions.push({
                label: item.text,
                value: item.value
            })
        })
