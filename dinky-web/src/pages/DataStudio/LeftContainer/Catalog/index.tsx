...
import {Button, Col, Empty, message, Modal, Row, Select, Spin, Tabs, Tooltip} from "antd";
import {getMSCatalogs, getMSFlinkColumns, getMSSchemaInfo} from "./service";
import {MetaStoreTableType} from "@/pages/DataStudio/model";
import {
    AppstoreOutlined,
    BlockOutlined, CodepenOutlined,
    DownOutlined,
    FunctionOutlined, ReloadOutlined,
    TableOutlined
...
import {clearMetaDataTable} from "@/pages/DataStudio/LeftContainer/MetaData/service";
...
export const Catalog: React.FC = (props: any) => {
    ...
    useEffect(() => {
        if (envId) {
            ...
            getMSCatalogs(param).then(d => {
                setCatalogSelect((d as any[]).map(item => {
                    setLoading(false)
                    return {
                        label: item.name,
    ...
    return (
        <Spin spinning={loading}>
            ...
            >
                <Tabs defaultActiveKey="tableInfo" size="small" items={[
                    {
                        key: 'tableInfo',
                        label: <span><TableOutlined/>{l('pages.datastudio.catalog.tableInfo')}</span>,
                        children: row ? <TableInfo tableInfo={row}/> : <Empty image={Empty.PRESENTED_IMAGE_SIMPLE}/>,
                    }, {
                        key: 'columnsInfo',
                        label: <span><CodepenOutlined/>{l('pages.datastudio.catalog.fieldInformation')}</span>,
