...
import {queryList, removeById} from "@/services/api";
...
import {ErrorMessage, ErrorMessageAsync} from "@/utils/messages";
...
const JobExecHistory : React.FC<connect> = (props) => {
    ...
    return <>
      <Paragraph>
        <>
          ...
          <Divider type="vertical"/>{l('global.table.startTime')}:{formatDateToYYYYMMDDHHMMSS(item.startTime)}
          <Divider type="vertical"/>{l('global.table.finishTime')}:{formatDateToYYYYMMDDHHMMSS(item.endTime)}
        </>
      </Paragraph>
    </>
  }
  ...
  const renderSubTitle = (item: JobExecutionHistory) => {
    ...
    return (
      <Space size={0}>
        ...
        {(item.status == 2) ?
          (<Space><Badge status="success"/><Text type="success">{l('global.job.status.success')}</Text></Space>) :
          (item.status == 1) ?
            <Space><Badge status="success"/><Text type="secondary">{l('global.job.status.running')}</Text></Space> :
            (item.status == 3) ?
              <Space><Badge status="error"/><Text type="danger">{l('global.job.status.failed')}</Text></Space> :
              (item.status == 4) ?
                <Space><Badge status="error"/><Text type="warning">{l('global.job.status.canceled')}</Text></Space> :
                (item.status == 0) ?
                  <Space><Badge status="error"/><Text type="warning">{l('global.job.status.initiating')}</Text></Space> :
                  <Space><Badge status="success"/><Text type="danger">{l('global.job.status.unknown')}</Text></Space>}
      </Space>
    );
  }
  ...
  return <>
    <ProList<JobExecutionHistory>
      ...
      request={(params, sorter, filter: any) => queryList(API_CONSTANTS.HISTORY_LIST, {
      ...{...params,taskId: currentTab?.params?.taskId}
        , sorter: {id: 'descend'}, filter})}
      pagination={{
        defaultPageSize: 5,
    ...
    <JobDetailInfoModel modalVisit={modalVisit} handleCancel={handleCancel} row={historyData} type={type} />  </>
};
