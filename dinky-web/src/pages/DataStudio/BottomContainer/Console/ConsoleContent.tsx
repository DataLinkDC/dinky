    ...
    const renderTitle = (node: any) => {
        ...
        return (
            <Space size={5}>
                ...
                {node.status == "FINISHED" && <CheckOutlined style={{color: 'green', fontWeight: 'bold'}}/>}
                {node.status == "FAILED" && <CloseCircleFilled style={{color: 'red', fontWeight: 'bold'}}/>}
                <Text>{node.title}</Text>
                <Text type={'secondary'} style={{marginLeft:'auto'}}>{parseMilliSecondStr(duration)}</Text>
            </Space>
        )
    }
