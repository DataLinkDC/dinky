...
const BottomContainer: React.FC<BottomContainerProps> = (props: any) => {
    ...
    const [tabState, setTabState] = useState<Record<string, Tab[]>>(Object.keys(BottomTabRoute).map(x => ({[x]: []})).reduce((a, b) => ({...a, ...b}), {}));
    ...
    const renderTabPane = () => {
        ...
        const onEdit = async (
            ...
            switch (action) {
                case 'add':
                    await BottomTabRoute[bottomContainer.selectKey].onAdd(tabState?.[bottomContainer.selectKey] ?? [], bottomContainer.selectKey, currentData, refresh);
                    break;
                case 'remove':
                    const newPanes = tabState?.[bottomContainer.selectKey].filter(pane => pane.key !== targetKey);
        ...
        return (
            <>
                ...
                {BottomTabRoute[bottomContainer.selectKey] &&
                    <Tabs
                        style={{height: '32px', display: '-webkit-box'}}
                        items={tabState?.[bottomContainer.selectKey].map(x => ({key: x.key, label: x.label}))}
                        onEdit={onEdit}
                        activeKey={
                            bottomContainer.selectKey +
                            '/'  +
    ...
    const renderItems = () => {
        return [
            ...
            ...getSubTabs(), ...tabState?.[bottomContainer.selectKey] ?? []
        ].map((item) => {
            return {
                ...item,
                children: (
    ...
    return (
        <MovableSidebar
            ...
            btnGroup={BottomBtnRoute[bottomContainer.selectKey]?.map(x => <CircleBtn key={x.key} icon={x.icon}
                                                                                     onClick={async () => {
                                                                                         const tabs = tabState?.[bottomContainer.selectKey] ?? [];
