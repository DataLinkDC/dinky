...
export default () => {
  ...
  return (
    <><ProList<any>
      ...
      metas={{
        ...
        actions: {
          render: (text, row) => [
            <Link
              to={`/dashboard/dashboard-layout/${row.id}`}
            >
              打开
            </Link>,
            <a
            ...
            <a target="_blank" rel="noopener noreferrer" key="view" onClick={async ()=>{
    ...
    />
      <Modal open={openCreate} onCancel={onCloseCreate} onClose={onCloseCreate} onOk={onAddOrUpdate}>
        <ProForm<DashboardData>
          params={{}}
          submitter={false}
          formRef={formRef}
