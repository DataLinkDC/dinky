...
const jsxTab = {
  ...
  title: 'jsx',
  closable: true,
  content: <iframe width={"100%"} height={"100%"} src={`https://ticlo.github.io/rc-dock/examples/basic.jsx.html`}/>
};
...
const htmlTab = {
  ...
  title: 'html',
  closable: true,
  content: <iframe width={"100%"} height={"100%"} src={`https://ticlo.github.io/rc-dock/examples/basic.html.html`}/>
};


const quickGuideTab: TabData = {
  closable: false, id: 'quick-start', title: '快速开始', content: (
    <div style={{height: 0}}>
      <KeyBoard/>
      <Divider/>
      <br/>
...
export const useLayout = (layoutState: LayoutState, setLayoutState: Dispatch<SetStateAction<LayoutState>>, dockLayoutRef: RefObject<DockLayout>) => {
  ...
  const activePanelOnClick = () => {
    // @ts-ignore
    const panels = Array.from(dockLayoutRef.current?._ref.querySelectorAll('.dock-box>.dock-panel'));
    for (const panel of panels) {
      ...
      if (position?.length == 2 && (position[1] === "leftTop" || position[1] === "right" || position[1] === "leftBottom")) {
        // 这里为panel添加点击事件
    ...
    children: [
      {
        ...
        children: [
          {
            id: "leftTop",
            tabs: [{content: <Project/>, id: 'project', title: '项目', minHeight: 50, group: "leftTop"}],
          },
          {
            size: 1000,
            tabs: [quickGuideTab],
...
const centerPanelExtraButtons = (panelData: PanelData, context: DockContext) => {
  ...
  if (panelData.parent?.mode !== 'window') {
    buttons.push(
      <SelectOutlined rotate={90} className='my-panel-extra-btn' key='new-window' title='在新窗口中打开'
    ...
    buttons.push(
      <MaximizeIcon className='my-panel-extra-btn' key='maximize'
    ...
    buttons.push(
      <ImportOutlined className='my-panel-extra-btn' key='move to dock' title='Dock'
                      onClick={() =>
                        // @ts-ignore
...
const toolbarPanelExtraButtons = (panelData: PanelData, context: DockContext) => {
  ...
  buttons.push(
    <CloseOutlined className='my-panel-extra-btn' key='close' title='关闭'
...
const toolbarPanelExtra = (panelData: PanelData, context: DockContext) => {
  ...
  return <>{toolbarPanelExtraButtons(panelData, context).map((button) => (
    button
  ))}</>
}
...
//  中间内容group
  'center-content': {
    ...
    panelExtra: (panelData: PanelData, context: DockContext) => {
      return <div>{centerPanelExtraButtons(panelData, context).map((button) => (
        button
      ))}</div>
    }
  }
