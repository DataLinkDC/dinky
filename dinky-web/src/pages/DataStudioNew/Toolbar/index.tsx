...
export default (props: ToolbarProp) => {
  ...
  return (<Flex wrap gap={4} justify={"center"} className={"toolbar-side"}>
    {route.map((item) => {
      // 设置显示样式
      ...
      return (
        <Row className={className} align={"middle"} justify={"center"} key={item.key} onClick={() => {
          onClick(item)
        }}>
        <span style={{
          width: "100%",
          textAlign: "center"
        }}>{React.cloneElement(item.icon, {className: "toolbar-icon"})}</span>
