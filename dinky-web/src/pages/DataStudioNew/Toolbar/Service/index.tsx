...
const Service = (props: { showDesc: boolean, tabs: CenterTab[], action: any }) => {
    ...
    return (<PanelGroup direction={"horizontal"}>
        <Panel defaultSize={20}>
            <Flex justify={"right"}>
                <RunToolBarButton showDesc={showDesc} desc={"全部折叠"} icon={<ShrinkOutlined/>} sleepTime={100} onClick={async () => {
                    setExpandKeys([])
                }}/>
                <RunToolBarButton showDesc={showDesc} desc={"全部展开"} icon={<ArrowsAltOutlined/>} sleepTime={100} onClick={async () => {
                    expandAll()
                }}/>
        ...
        {selectedKey && selectedKey.length > 0 && selectedKey[0] !== undefined && <Panel style={{paddingInline: 10}}>
            <KeepAlive cacheKey={"service:" + selectedKey[0]} when={() => false}>
                {renderContent()}
            </KeepAlive>
        </Panel>}
