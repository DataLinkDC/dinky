...
export const LineageNew = memo((props: { data: LineageDetailInfo }) => {
  ...
  const tablesMap = data.tables.reduce((acc, item) => {
  ...
  return <Graphin
    ...
    options={{
      autoResize: true,
      data: {
        nodes: data.tables.flatMap(item => (item.columns.map(column => ({
      ...
      layout: {
        ...
        outerLayout: new DagreLayout({rankdir: 'LR', edgeLabelSpace: false, nodesep: 5, ranksep: 50}),
      },
