...
import {DownloadOutlined, QuestionCircleOutlined, SearchOutlined, SyncOutlined} from '@ant-design/icons';
...
export default (props: { taskId: number; action: any; dialect: string }) => {
  ...
  const renderDownloadButton = () => {
    if (data.columns) {
      ...
      return (
        <Button type='link' href={url} icon={<DownloadOutlined/>} title={'Export Csv'}/>
      );
    }
    return undefined;
  };
  const renderAVA = () => {
    return (
      <Button type='link' title={'自动洞察'} onClick={() => {
        setOpenAVA(true)
        startTransition(() => {
  ...
  return (
    <div style={{width: '100%', paddingInline: 10}}>
      ...
      <Modal open={openAVA} loading={isPending} width={"80%"} onClose={handleCloseAva} onCancel={handleCloseAva}
             footer={[<Button key="submit" type="primary" loading={loading} onClick={handleCloseAva}>
               Ok
             </Button>]}>
        <div key="plot" style={{flex: 5, height: '100%'}}>
          {avaResult?.insights &&
            avaResult.insights.map((insight, index) => {
              return <InsightCard insightInfo={insight} key={index} visualizationOptions={{lang: 'zh-CN'}}/>;
            })}
        </div>
      </Modal>
    </div>
