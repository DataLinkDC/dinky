...
const DataStudioNew: React.FC = () => {
  ...
  const handleMenuClick = (values: MenuInfo) => {
    ...
    switch (values.key) {
      case 'showToolbarDesc':
        setLayoutState(prevState => ({...prevState, toolbar: {...prevState.toolbar, showDesc: true}}));
        break;
      case 'hideToolbarDesc':
        setLayoutState(prevState => ({...prevState, toolbar: {...prevState.toolbar, showDesc: false}}));
        break;
        case 'showToolbarActiveTab':
        setLayoutState(prevState => ({...prevState, toolbar: {...prevState.toolbar, showActiveTab: true}}));
        break;
      case 'hideToolbarActiveTab':
        setLayoutState(prevState => ({...prevState, toolbar: {...prevState.toolbar, showActiveTab: false}}));
        break;
      case 'saveLayout':
    ...
    setLayoutState(prevState => {
      ...
      if (prevState.toolbar[route.position].currentSelect === route.key) {
        ...
        prevState.toolbar[route.position] = {
          ...
          allTabs: new Set([...prevState.toolbar[route.position]?.allTabs ?? []].filter(t => t !== route.key))
        }
      } else {
        // 新增tab
        if (tab && !newTab) {
          ...
          dockLayoutRef.current?.dockMove({
            ...
            title: route.title,
            group: route.position
          }, dockLayoutRef.current?.getLayout().dockbox, getDockPositionByToolbarPosition(route.position))
  ...
  return (
    <PageContainer breadcrumb={undefined} title={false} childrenContentStyle={{margin: 0, padding: 0}}>
      <Row style={{height: "calc(100vh - 81px)"}}>
        {/*左边工具栏*/}
        <Col style={{width: toolbarWidth, height: "inherit"}} flex="none"
             onContextMenu={rightContextMenuHandle}>
          {/*左上工具栏*/}
          <Col style={{width: "inherit", height: "50%"}}>
            <Toolbar showDesc={layoutState.toolbar.showDesc}
        ...
        <Col style={{height: "inherit"}} flex="auto">
          <DockLayout ref={dockLayoutRef} defaultLayout={layout} groups={groups}
                      style={{position: 'absolute', left: 0, top: 0, right: 0, bottom: 0}}
                      onLayoutChange={onLayoutChange} saveTab={saveTab}/>
        </Col>
