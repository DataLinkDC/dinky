 ...
 * @param subTasks
 */
export const buildMetricsList = (subTasks: string[]) => subTasks.map((subTask) => {
    // build label
    let label = <div className={'tag-content'}>
        <Tag icon={<SyncOutlined spin/>} color={'processing'}>{l('metrics.flink.metrics.name')}: {subTask}</Tag>
    </div>;
 ...
 */
export const buildRunningJobList = (metrics: Task[]) => metrics.map((item) => {
    // build label
    let label = <div className={'tag-content'}>
        <Tag color={'processing'}>{l('metrics.flink.jobId')}: {item.jid}</Tag>
        <Tag color={'success'}>{l('metrics.flink.taskId')}: {item.id}</Tag>
        <Tag color={'success'}>{l('metrics.flink.job.name')}: {item.name}</Tag>
    </div>;
