    ...
    return (
      <>
        <Descriptions
          ...
          items={items}
          extra={
            <Button type={'link'} onClick={() => run(instance?.taskId, ckp.external_path,l('devops.jobinfo.ck.recovery'))}>
              重启后此处恢复
            </Button>
          }
        />
      ...
      <Spin spinning={loading}>
        {instance?.status && !isStatusDone(instance?.status) ? (
          <>
            ...
            <Button type={'link'} size={'small'} onClick={() => run(instance?.taskId, '', l('devops.jobinfo.ck.recovery'))}>
