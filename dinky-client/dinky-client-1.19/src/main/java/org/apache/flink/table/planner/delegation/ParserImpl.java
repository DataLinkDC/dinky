...
public class ParserImpl implements Parser {
    ...
    public List<Operation> parse(String statement) {
        ...
        return Collections.singletonList(SqlNodeToOperationConversion.convert(planner, catalogManager, parsed.get(0))
    ...
    @Override
    public RelNode parseSqlToRelNode(String statement) {
        CalciteParser parser = calciteParserSupplier.get();
        FlinkPlannerImpl planner = validatorSupplier.get();
        ...
        Preconditions.checkArgument(parsed.size() == 1, "only single statement supported");
        SqlNode validated;
        if (parsed.get(0) instanceof RichSqlInsert) {
            RichSqlInsert sqlInsert = (RichSqlInsert) parsed.get(0);
            validated = planner.validate(sqlInsert.getSource());
        } else {
            validated = planner.validate(parsed.get(0));
        }
        final RelRoot rel = planner.rel(validated);
        return rel.project();
    }
    ...
    public ResolvedExpression parseSqlExpression(
        ...
        try {
            final SqlToRexConverter sqlToRexConverter = rexFactory.createSqlToRexConverter(inputRowType, outputType);
