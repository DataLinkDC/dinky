...
public class YarnClusterDescriptor implements ClusterDescriptor<ApplicationId> {
    ...
    private Optional<List<Path>> decodeFilesToShipToCluster(
        ...
        List<Path> files = ConfigUtils.decodeListFromConfig(configuration, configOption, this::createPathWithSchema);
    ...
    private Optional<Path> getLocalFlinkDistPath(final Configuration configuration) {
        ...
        return decodedPath.endsWith(".jar") ? Optional.of(getPathFromLocalFilePathStr(decodedPath)) : Optional.empty();
    }
    ...
    private String getDecodedJarPath() {
        ...
        try {
            ...
            throw new RuntimeException("Couldn't decode the encoded Flink dist jar path: "
                    + encodedJarPath
    ...
    private static boolean isArchiveOnlyIncludedInShipArchiveFiles(
        ...
        long archivedFileCount = shipFiles.stream()
                .map(FunctionUtils.uncheckedFunction(path -> getFileStatus(path, yarnConfiguration)))
    ...
    private void isReadyForDeployment(ClusterSpecification clusterSpecification) throws Exception {
        ...
        int configuredVcores =
                flinkConfiguration.get(YarnConfigOptions.VCORES, clusterSpecification.getSlotsPerTaskManager());
    ...
    @Override
    public ClusterClientProvider<ApplicationId> deploySessionCluster(ClusterSpecification clusterSpecification)
            throws ClusterDeploymentException {
        try {
            return deployInternal(
                    clusterSpecification, "Flink session cluster", getYarnSessionClusterEntrypoint(), null, false);
        } catch (Exception e) {
    ...
    public ClusterClientProvider<ApplicationId> deployJobCluster(
        ...
        try {
            return deployInternal(
                    clusterSpecification, "Flink per-job cluster", getYarnJobClusterEntrypoint(), jobGraph, detached);
        } catch (Exception e) {
        ...
        try {
            ...
            try (final FileSystem fs = FileSystem.get(yarnConfiguration)) {
                final Path applicationDir =
                        YarnApplicationFileUploader.getApplicationDirPath(getStagingDir(fs), applicationId);
    ...
    private ClusterClientProvider<ApplicationId> deployInternal(
        ...
        if (HadoopUtils.isKerberosSecurityEnabled(currentUser)) {
            boolean useTicketCache = flinkConfiguration.get(SecurityOptions.KERBEROS_LOGIN_USETICKETCACHE);
            ...
            final boolean fetchToken = flinkConfiguration.get(SecurityOptions.KERBEROS_FETCH_DELEGATION_TOKEN);
            final boolean yarnAccessFSEnabled = !CollectionUtil.isNullOrEmpty(
                    flinkConfiguration.get(SecurityOptions.KERBEROS_HADOOP_FILESYSTEMS_TO_ACCESS));
        ...
        flinkConfiguration.set(ClusterEntrypoint.INTERNAL_CLUSTER_EXECUTION_MODE, executionMode.toString());
    ...
    private ClusterSpecification validateClusterResources(
        ...
        if (taskManagerMemoryMb > freeClusterResources.containerLimit) {
            LOG.warn("The requested amount of memory for the TaskManagers ("
                    + taskManagerMemoryMb
                    + "MB) is more than "
        ...
        if (jobManagerMemoryMb > freeClusterResources.containerLimit) {
            LOG.warn("The requested amount of memory for the JobManager ("
                    + jobManagerMemoryMb
                    + "MB) is more than "
                    + "the largest possible YARN container: "
                    + freeClusterResources.containerLimit
    ...
    private void logIfComponentMemNotIntegerMultipleOfYarnMinAllocation(
        ...
        int normalizedMemMB =
                (componentMemoryMB + (yarnMinAllocationMB - 1)) / yarnMinAllocationMB * yarnMinAllocationMB;
    ...
    private void checkYarnQueues(YarnClient yarnClient) {
        try {
            ...
            if (queues.size() > 0
                    && this.yarnQueue != null) { // check only if there are queues configured in yarn and for
                // this session.
    ...
    private ApplicationReport startAppMaster(
        ...
        if (!fs.getClass().getSimpleName().equals("GoogleHadoopFileSystem")
            ...
            LOG.warn("The file system scheme is '"
                    + fs.getScheme()
        ...
        final List<Path> providedLibDirs = Utils.getQualifiedRemoteProvidedLibDirs(configuration, yarnConfiguration);
        ...
        final YarnApplicationFileUploader fileUploader = YarnApplicationFileUploader.from(
                stagingDirFs, stagingDirPath, providedLibDirs, appContext.getApplicationId(), getFileReplication());

        // The files need to be shipped and added to classpath.
        Set<Path> systemShipFiles = new HashSet<>(shipFiles);

        final String logConfigFilePath = configuration.get(YarnConfigOptionsInternal.APPLICATION_LOG_CONFIG_FILE);
        ...
        if (HighAvailabilityMode.isHighAvailabilityModeActivated(configuration)) {
            ...
            appContext.setMaxAppAttempts(configuration.getInteger(
                    YarnConfigOptions.APPLICATION_ATTEMPTS.key(), YarnConfiguration.DEFAULT_RM_AM_MAX_ATTEMPTS));
        ...
        final List<String> uploadedDependencies =
                fileUploader.registerMultipleLocalResources(systemShipFiles, Path.CUR_DIR, LocalResourceType.FILE);
        ...
        if (providedLibDirs == null || providedLibDirs.isEmpty()) {
            ...
            fileUploader.registerMultipleLocalResources(shipOnlyFiles, Path.CUR_DIR, LocalResourceType.FILE);
        }

        if (!shipArchives.isEmpty()) {
            fileUploader.registerMultipleLocalResources(shipArchives, Path.CUR_DIR, LocalResourceType.ARCHIVE);
        }
        ...
        if (providedUsrLibDir.isPresent()) {
            final List<String> usrLibClassPaths = fileUploader.registerMultipleLocalResources(
                    Collections.singletonList(providedUsrLibDir.get()), Path.CUR_DIR, LocalResourceType.FILE);
            ...
            final List<String> usrLibClassPaths = fileUploader.registerMultipleLocalResources(
                    usrLibShipFiles.stream().map(e -> new Path(e.toURI())).collect(Collectors.toSet()),
                    Path.CUR_DIR,
                    LocalResourceType.FILE);
            userClassPaths.addAll(usrLibClassPaths);
        }
        ...
        if (userJarInclusion == YarnConfigOptions.UserJarInclusion.FIRST) {
            for (String userClassPath : userClassPaths) {
                classPathBuilder.append(userClassPath).append(pathSeparator);
            }
        }
        for (String classPath : systemClassPaths) {
            classPathBuilder.append(classPath).append(pathSeparator);
        }
        ...
        if (jobGraph != null) {
            ...
            try {
                ...
                fileUploader.registerSingleLocalResource(
                        jobGraphFilename, new Path(tmpJobGraphFile.toURI()), "", LocalResourceType.FILE, true, false);
                classPathBuilder.append(jobGraphFilename).append(pathSeparator);
            } catch (Exception e) {
        ...
        try {
            ...
            fileUploader.registerSingleLocalResource(
                    ...
                    true,
                    true);
            classPathBuilder.append(flinkConfigFileName).append(pathSeparator);
        } finally {
        ...
        if (System.getenv("IN_TESTS") != null) {
            ...
            remoteYarnSiteXmlPath = fileUploader
                    .registerSingleLocalResource(
                            Utils.YARN_SITE_FILE_NAME, yarnSitePath, "", LocalResourceType.FILE, false, false)
                    .getPath();
        ...
        if (!StringUtils.isNullOrWhitespaceOnly(krb5Config)) {
            ...
            remoteKrb5Path = fileUploader
                    .registerSingleLocalResource(
                            Utils.KRB5_FILE_NAME, krb5ConfPath, "", LocalResourceType.FILE, false, false)
                    .getPath();
            hasKrb5 = true;
        }
        ...
        if (keytab != null) {
            ...
            if (localizeKeytab) {
                ...
                remotePathKeytab = fileUploader
                        .registerSingleLocalResource(
                                localizedKeytabPath, new Path(keytab), "", LocalResourceType.FILE, false, false)
                        .getPath();
            } else {
        ...
        final Map<String, String> appMasterEnv = generateApplicationMasterEnv(
                fileUploader, classPathBuilder.toString(), localResourceDescFlinkJar.toString(), appId.toString());
    ...
    private void setTokensFor(ContainerLaunchContext containerLaunchContext, boolean fetchToken) throws Exception {
    ...
    Path getStagingDir(FileSystem defaultFileSystem) throws IOException {
        final String configuredStagingDir = flinkConfiguration.get(YarnConfigOptions.STAGING_DIRECTORY);
    ...
    private int getFileReplication() {
        final int yarnFileReplication =
                yarnConfiguration.getInt(DFSConfigKeys.DFS_REPLICATION_KEY, DFSConfigKeys.DFS_REPLICATION_DEFAULT);
    ...
    private static String encodeYarnLocalResourceDescriptorListToString(List<YarnLocalResourceDescriptor> resources) {
        return String.join(
                ...
                resources.stream().map(YarnLocalResourceDescriptor::toString).collect(Collectors.toList()));
    }
    ...
    private ClusterResourceDescription getCurrentFreeClusterResources(YarnClient yarnClient)
        ...
        for (int i = 0; i < nodes.size(); i++) {
            ...
            long free = rep.getCapability().getMemorySize()
    ...
    public String getClusterDescription() {
        ...
        try {
            ...
            ps.println("Summary: totalMemory " + getDisplayMemory(totalMemory) + " totalCores " + totalCores);
    ...
    private void activateHighAvailabilitySupport(ApplicationSubmissionContext appContext)
        ...
        reflector.setAttemptFailuresValidityInterval(
                appContext, flinkConfiguration.get(YarnConfigOptions.APPLICATION_ATTEMPT_FAILURE_VALIDITY_INTERVAL));
    }
    ...
    private static class ApplicationSubmissionContextReflector {
        ...
        public void setApplicationTags(ApplicationSubmissionContext appContext, Set<String> applicationTags)
                throws InvocationTargetException, IllegalAccessException {
        ...
        public void setApplicationNodeLabel(ApplicationSubmissionContext appContext, String nodeLabel)
                throws InvocationTargetException, IllegalAccessException {
        ...
        public void setAttemptFailuresValidityInterval(ApplicationSubmissionContext appContext, long validityInterval)
                throws InvocationTargetException, IllegalAccessException {
    ...
    void addLibFoldersToShipFiles(Collection<Path> effectiveShipFiles) {
        ...
        if (libDir != null) {
            ...
            if (directoryFile.isDirectory()) {
                ...
                throw new YarnDeploymentException("The environment variable '"
                        + ENV_FLINK_LIB_DIR
                        + "' is set to '"
                        + libDir
    ...
    void addUsrLibFolderToShipFiles(Collection<File> effectiveShipFiles) {
        ...
        ClusterEntrypointUtils.tryFindUserLibDirectory().ifPresent(usrLibDirFile -> {
            ...
            LOG.info("usrlib: {} will be shipped automatically.", usrLibDirFile.getAbsolutePath());
        });
    }
    ...
    ContainerLaunchContext setupApplicationMasterContainer(
        ...
        final String commandTemplate = flinkConfiguration.get(YARN_CONTAINER_START_COMMAND_TEMPLATE);
    ...
    private void setHAClusterIdIfNotSet(Configuration configuration, ApplicationId appId) {
        ...
        if (!configuration.contains(HighAvailabilityOptions.HA_CLUSTER_ID)) {
            configuration.set(HighAvailabilityOptions.HA_CLUSTER_ID, ConverterUtils.toString(appId));
        }
    }
    ...
    Map<String, String> generateApplicationMasterEnv(
        ...
        env.putAll(ConfigurationUtils.getPrefixedKeyValuePairs(
                ResourceManagerOptions.CONTAINERIZED_MASTER_ENV_PREFIX, this.flinkConfiguration));
        ...
        env.put(
                ...
                encodeYarnLocalResourceDescriptorListToString(fileUploader.getEnvShipResourceList()));
        env.put(
