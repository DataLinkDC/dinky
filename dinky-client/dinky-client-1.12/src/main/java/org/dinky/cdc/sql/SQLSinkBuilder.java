...
public class SQLSinkBuilder extends AbstractSinkBuilder implements SinkBuilder, Serializable {
                    ...
                    public void flatMap(Map value, Collector<Row> out) throws Exception {
                        try {
                            switch (value.get("op").toString()) {
                                case "r":
                                case "c":
                                    Row irow = new Row(RowKind.INSERT, columnNameList.size());
                                    ...
                                    break;
                                case "d":
                                    Row drow = new Row(RowKind.DELETE, columnNameList.size());
                                ...
                                case "u":
                                    Row ubrow =
                                            new Row(RowKind.UPDATE_BEFORE, columnNameList.size());
                                    ...
                                    out.collect(ubrow);
                                    Row uarow =
                                            new Row(RowKind.UPDATE_AFTER, columnNameList.size());
    ...
    private void addTableSink(
        ...
        customTableEnvironment.createTemporaryView(
                viewName, rowDataDataStream, StringUtils.join(columnNameList, ","));
        logger.info("Create " + viewName + " temporaryView successful...");
        String flinkDDL =
                FlinkBaseUtil.getFlinkDDL(
                        table, sinkTableName, config, sinkSchemaName, sinkTableName, pkList);
