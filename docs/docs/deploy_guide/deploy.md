...
|- application.yml
extends/ -- 扩展
|- dinky-client-1.11.jar -- 适配 Flink1.11.x
|- dinky-client-1.12.jar -- 适配 Flink1.12.x
|- dinky-client-1.14.jar -- 适配 Flink1.14.x
|- dinky-client-1.15.jar -- 适配 Flink1.15.x
html/ -- 前端编译产物
jar/ 
  |- dinky-app-1.11.jar -- dinky application 模式提交 sql 用到的 jar 适配 Flink1.11.x
  |- dinky-app-1.12.jar -- dinky application 模式提交 sql 用到的 jar 适配 Flink1.12.x
  |- dinky-app-1.13.jar -- dinky application 模式提交 sql 用到的 jar 适配 Flink1.13.x
  |- dinky-app-1.14.jar -- dinky application 模式提交 sql 用到的 jar 适配 Flink1.14.x
  |- dinky-app-1.15.jar -- dinky application 模式提交 sql 用到的 jar 适配 Flink1.15.x
  |- dinky-client-base.jar  -- 整库同步场景下需要的包 
  |- dinky-common.jar -- 整库同步场景下需要的包
lib/ -- 内部组件
|- dinky-alert-dingtalk.jar 
|- dinky-alert-email.jar 
|- dinky-alert-feishu.jar 
|- dinky-alert-wechat.jar 
|- dinky-client-1.13.jar  -- 适配 Flink1.13.x,默认
|- dinky-catalog-mysql.jar -- dinky 的 catalog 实现 
|- dinky-connector-jdbc.jar
|- dinky-function.jar
|- dinky-metadata-clickhouse.jar
|- dinky-metadata-doris.jar
|- dinky-metadata-hive.jar
|- dinky-metadata-mysql.jar
|- dinky-metadata-oracle.jar
|- dinky-metadata-phoenix.jar
|- dinky-metadata-postgresql.jar
|- dinky-metadata-sqlserver.jar
plugins/
...
|- upgrade/ -- 各个版本的升级 SQL
auto.sh --启动停止脚本
dinky-admin.jar --主程序包
```

#### flink 版本适配

**dinky-catalog-mysql**、**dinky-client**、**dinky-app**。
...
比如 **lib** 目录下的 **dinky-client-1.14-0.6.7.jar** ，表示使用的 flink 版本为 1.14.x ，
如果你在 **plugins** 目录下上传的 flink 用到的 jar 包的版本不是 1.14.x ，就需要更换 **dinky-client** 包。
