...
public class FlinkStatementUtil {
    ...
    public static String getCDCInsertSql(Table table, String targetName, String sourceName, FlinkCDCConfig config) {
        ...
        for (int i = 0; i < table.getColumns().size(); i++) {
            ...
            sb.append(getColumnProcessing(table.getColumns().get(i), config))
                    .append(" \n");
        }
        sb.append(" FROM `");
    ...
    public static String getColumnProcessing(Column column, FlinkCDCConfig config) {
        ...
        if (configType.contains("postgres-cdc") && (columnType.contains("numeric") || columnType.contains("decimal"))
