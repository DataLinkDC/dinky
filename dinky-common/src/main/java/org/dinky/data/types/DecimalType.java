...
public final class DecimalType extends LogicalType {
    ...
    public LogicalType copy(LogicalTypeParam param) {
        ...
        return new DecimalType(param.getNullable(), param.getPrecision(), param.getScale());
    }

    public LogicalType copy(boolean isNullable, Integer precision, Integer scale) {
        if (precision == null && scale == null) {
            return copy(isNullable);
        } else if (precision == null) {
            return new DecimalType(isNullable, this.precision, scale);
        } else if (scale == null) {
            return new DecimalType(isNullable, precision, this.scale);
        }
        return new DecimalType(isNullable, precision, scale);
    }
