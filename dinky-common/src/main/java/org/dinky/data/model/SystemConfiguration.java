...
public class SystemConfiguration {
    ...
    private final Configuration<Integer> flinkHistoryServerPort = key(Status.SYS_FLINK_SETTINGS_FLINK_HISTORY_SERVER_PORT)
            .intType()
            .defaultValue(8082)
            .note(Status.SYS_FLINK_SETTINGS_FLINK_HISTORY_SERVER_PORT_NOTE);
    private final Configuration<Integer> flinkHistoryServerArchiveRefreshInterval = key(Status.SYS_FLINK_SETTINGS_FLINK_HISTORY_SERVER_ARCHIVE_REFRESH_INTERVAL)
            .intType()
            .defaultValue(5000)
    ...
    public Map<String,String> getFlinkHistoryServerConfiguration(){
        ...
        if (useFlinkHistoryServer.getValue()){
            config.put("historyserver.web.port",flinkHistoryServerPort.getValue().toString());
            config.put("historyserver.archive.fs.refresh-interval",flinkHistoryServerArchiveRefreshInterval.getValue().toString());
            config.put("historyserver.web.tmpdir",FileUtil.file(DirConstant.getTempRootDir(),"flink-job-archive").getAbsolutePath());
