...
public class CABuilder {
    ...
    public static List<TableCANode> getOneTableColumnCAByStatement(String statement) {
        ...
        for (int j = 0; j < results.size(); j++) {
            ...
            TableCA sinkTableCA = (TableCA) result.getSinkTableCA();
            TableCANode node =
                    new TableCANode(id++, sinkTableCA.getTableName(), sinkTableCA.getFields());
            List<TableCANode> children = new ArrayList<>();
            for (int k = 0; k < result.getSourceTableCAS().size(); k++) {
        ...
        for (int j = 0; j < columnCAResults.size(); j++) {
            ...
            for (int k = 0; k < sinkColumns.size(); k++) {
                ...
                buildColumnCANodeChildren(
                        children, result, sinkColumns.get(k), columnCA.getOperation());
                node.setChildren(children);
    ...
    private static void buildColumnCANodeChildren(
            List<ColumnCANode> children,
            ColumnCAResult result,
            Integer columnId,
            String operation) {
            ...
            if (columnId.equals(nodeRel.getSufId())) {
                ...
                operation =
                        operation.replaceAll(
                                childca.getAlias().replaceAll("\\)", ""), childca.getOperation());
