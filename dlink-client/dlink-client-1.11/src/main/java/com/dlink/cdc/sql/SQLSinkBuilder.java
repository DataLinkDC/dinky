                    ...
                    public void flatMap(Map value, Collector<Row> out) throws Exception {
                        try {
                            switch (value.get("op").toString()) {
                                case "r":
                                case "c":
                                    Row irow = Row.ofKind(RowKind.INSERT, columnNameList.size());
                                    ...
                                    break;
                                case "d":
                                    Row drow = Row.ofKind(RowKind.DELETE, columnNameList.size());
                                    ...
                                    break;
                                case "u":
                                    Row ubrow = Row.ofKind(RowKind.UPDATE_BEFORE, columnNameList.size());
                                    Map ubdata = (Map) value.get("before");
                                    for (int i = 0; i < columnNameList.size(); i++) {
                                        ubrow.setField(i, convertValue(ubdata.get(columnNameList.get(i)), columnTypeList.get(i)));
                                    }
                                    out.collect(ubrow);
                                    Row uarow = Row.ofKind(RowKind.UPDATE_AFTER, columnNameList.size());
