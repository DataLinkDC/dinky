      ...
      description: (
        editName != 'sqlSeparator' ?
          (sqlSeparator ? sqlSeparator : l('pages.settings.FlinkNoSetting','未设置')) : (<Input
            ...
            onChange={onChange}
            placeholder=";"/>)),
      actions: editName != 'sqlSeparator' ? [<a onClick={({}) => handleEditClick('sqlSeparator')}>{l('pages.settings.FlinkUpdate','修改')}</a>] :
        ...
        editName != 'jobIdWait' ?
          (jobIdWait ? jobIdWait : '30') : (
            <Input
              ...
              onChange={onChange}
              placeholder="30"/>)),
      actions: editName != 'jobIdWait' ? [<a onClick={({}) => handleEditClick('jobIdWait')}>{l('pages.settings.FlinkUpdate','修改')}</a>] :
