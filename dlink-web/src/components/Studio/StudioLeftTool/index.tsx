...
const StudioLeftTool = (props: any) => {
  ...
  return (
    <Tabs defaultActiveKey="1" size="small" tabPosition="left" style={{height: toolHeight}}>
      ...
      <TabPane tab={<span><InsertRowAboveOutlined/> {l('pages.datastudio.label.structure', '结构')}</span>}
               key="MetaStore">
        <StudioMetaStore/>
      </TabPane>
      ...
      <TabPane tab={<span><DatabaseOutlined/> {l('pages.datastudio.label.datasource', '数据源')}</span>}
               key="DataSource">
        <StudioDataBase/>
      </TabPane>
      ...
      <TabPane tab={<span><CloudOutlined/> {l('pages.datastudio.label.globalvariable', '全局变量')}</span>}
               key="fragment">
        <StudioFragment/>
      </TabPane>
