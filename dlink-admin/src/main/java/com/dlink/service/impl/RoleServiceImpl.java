...
public class RoleServiceImpl extends SuperServiceImpl<RoleMapper, Role> implements RoleService {
    ...
    public Result saveOrUpdateRole(JsonNode para) {
        ...
        JsonNode namespaceJsonNode = para.get("namespaceId");
        ...
        JsonNode resourceJsonNode = para.get("resourceId");
        for (JsonNode ids : resourceJsonNode) {
            RoleResource roleResource = new RoleResource();
            roleResource.setRoleId(role.getId());
            roleResource.setResourceId(ids.asInt());
            roleResourceList.add(roleResource);
        }
        // save or update role resource relation
        boolean roleResourceResult = roleResourceService.saveOrUpdateBatch(roleResourceList, 1000);

        if (roleResult && roleNamespaceResult && roleResourceResult) {
    ...
    public Result deleteRoleById(JsonNode para) {
        for (JsonNode item : para) {
            ...
            if (roleResourceProTableResult.getData().size() > 0) {
                return Result.failed("删除角色失败，该角色已绑定资源");
            }
